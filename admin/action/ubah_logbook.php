...
try {
    ...
    // Check if exists
    $check_stmt = $conn->prepare("SELECT id FROM logbook WHERE id = ?");
    ...
    // Update database
    $stmt = $conn->prepare("UPDATE logbook SET 
    ...
    $stmt->bind_param(
        ...
        $tanggal, $koordinator, $alamat, $produk_json,
        $cash_dp, $satu_minggu, $satu_bulan, $jumlah_total,
        $presenter, $marketing, $komisi, $keterangan, $id
    );

    if ($stmt->execute()) {
        $_SESSION['success_message'] = 'Logbook berhasil diupdate!';
        echo json_encode([
            'success' => true,
